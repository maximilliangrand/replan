...
```mermaid
flowchart LR
  Operator[Operator in local React UI] --> App[TypeScript API and executor]
...
| ----------- | ---------------------------------------- | -------------------------------------------------------------------------------------------- |
| Application | `scenario_state`                         | Current synthetic scenario, start time, observed inventory and crash-control flag            |
| Application | `plans`                                  | Immutable proposal payload, source snapshot, strategy, hash, lifecycle status and timestamps |
| Application | `approvals`                              | Exact plan hash, approval time and local demo actor                                          |
...
The application serializes mutations with a PostgreSQL session advisory lock held
on a dedicated connection. A competing operation receives a conflict instead of
interleaving the workflow. A partial unique index also permits only one approved,
executing or uncertain plan per scenario. This is deliberately a single-operator,
small-workload design; it is not a general distributed scheduler.
...
database administrator rewriting all related records. The approval actor is the
literal local **Demo operator**, not an authenticated human identity.
...
carrier evidence or manual investigation. The MVP has no manual override UI or
provider cancellation protocol. A stuck ambiguous operation may therefore require
operator investigation outside the app; it is intentionally not guessed away.
...
This is a localhost demonstration with no authentication, tenant isolation,
authorization roles or production secrets. The API checks loopback Host and
Origin values as a local browser safeguard. Those checks do not replace identity
or authorization, and the simulator APIs are not a public deployment surface.
Do not expose these services as a shared operational system.
...
events are not cryptographically tamper-evident. The recovery tests instead restart
